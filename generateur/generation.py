...
def tiffEnCouleurs(fichier):
	...
	while t.count(0)!=0:								#Tant que des distances valent 0, on boucle
	...
	data[ d[0] , d[1] ] = [mer[0],mer[1],mer[2]]

	
	img = Image.fromarray(data, 'RGB')					#Crée une image à partir de la matrice
	img = img.convert('CMYK')							#Convertit en RGB pour être affichable dans le navigateur
	img.save('./images/carte.tiff')					#Enregistre l'image
	img.close();										#Fermeture de l'image
