...
app.post('/index/gen/',urlencodedParser, function (req,res){					//Génération de la miniature
	...
	if(req.body.telecharger=="telecharger"){						//On vérifie si le bouton télécharge a été clické
		...
		const ls = spawn('./generateur/minigen.py', [tv['nom'],tv['s1'],tv['c1'],tv['s2'],tv['c2'],tv['s3'],tv['c3'],tv['s4'],tv['c4'],tv['s5'],tv['c5']]);
		var donnees = '';
		...
		pythonShell.run('generation.py', options, function (err, results) {	//Génère la carte.tiff pour un futur téléchargement
			if (err) throw err;
			res.send(donnees);
		});
	}
